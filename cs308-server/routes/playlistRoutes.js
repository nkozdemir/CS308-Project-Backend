...
// Route to create a playlist with a list of song ids
router.post('/createPlaylistWithSongs', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;
    ...
    if (!playlistName || !songIDs) {
      return res.status(400).json({
        status: 'error',
        code: 400,
        message: 'Invalid parameters. Playlist name and song IDs must be provided.',
      });
    }
    const playlist = await playlistController.createPlaylist(playlistName, userId);
